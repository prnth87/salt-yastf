...
- **One SOQL per merge group, not one SOQL per handler.** The
  primary win. SALT issues 11 SOQL where fflib/O'Hara issue 20 and
  TAF issues 29 on the same workload (45 % / 62 % reductions).
...
|---|---|---|---|---|
| **SOQL** | **11** | 20 | 20 | 29 |
| Merge groups collapsed | 5 → 5 SOQL | 5 → ~15 SOQL | 5 → ~15 SOQL | 5 → ~15 SOQL |
